...
* **Hybrid Encryption (Sender):** Secures files using AES-256 (EAX mode) and securely wraps the temporary AES key using the receiver's RSA Public Key (PKCS1_OAEP).
* **Hybrid Decryption (Receiver):** Unlocks the AES key using the RSA Private Key, verifies data integrity (via MAC tag), and recovers the original file.
* **Live Demo Generator:** Includes a tool to instantly generate a 5MB readable text file for live presentation purposes.
* **Automated Performance Testing:** Includes a testing suite to measure AES processing speeds under moderate (50MB) and stress (200MB) loads.
